...
class HexagonalGridCalculatorImpl<SatelliteData>(override val hexagonalGrid: HexagonalGrid<SatelliteData>) : HexagonalGridCalculator<SatelliteData> {

    override fun calculateDistanceBetween(hex0: Hexagon<SatelliteData>, hex1: Hexagon<SatelliteData>): Int {
    ...
    override fun calculateMovementRangeFrom(hexagon: Hexagon<SatelliteData>, distance: Int): Set<Hexagon<SatelliteData>> {
        val ret = HashSet<Hexagon<SatelliteData>>()
        for (x in -distance..distance) {
            for (y in max(-distance, -x - distance)..min(distance, -x + distance)) {
                ...
                ret += hexagonalGrid.getByCubeCoordinate(tempCoordinate)
            }
        }
        return ret
    }

    override fun rotateHexagon(originalHex: Hexagon<SatelliteData>, targetHex: Hexagon<SatelliteData>, rotationDirection: RotationDirection): Hexagon<SatelliteData> {
    ...
    override fun calculateRingFrom(centerHexagon: Hexagon<SatelliteData>, radius: Int): Set<Hexagon<SatelliteData>> {
        val result = HashSet<Hexagon<SatelliteData>>()
    ...
    override fun drawLine(from: Hexagon<SatelliteData>, to: Hexagon<SatelliteData>): List<Hexagon<SatelliteData>> {
        val distance = calculateDistanceBetween(from, to)
        if (distance == 0) {
            return emptyList()
        }
        val results = ArrayList<Hexagon<SatelliteData>>(distance+1)
        for (i in 0..distance) {
            val interpolatedCoordinate = cubeLinearInterpolate(from.cubeCoordinate,
                    to.cubeCoordinate, 1.0 / distance * i)
            results.add(hexagonalGrid.getByCubeCoordinate(interpolatedCoordinate))
        }
        return results
    }

    override fun isVisible(from: Hexagon<SatelliteData>, to: Hexagon<SatelliteData>): Boolean {
        val traversePath = drawLine(from, to)
        for (pathHexagon in traversePath) {
            if (pathHexagon.equals(from) || pathHexagon.equals(to)) {
                continue
            }
            if (pathHexagon.satelliteData != null) { // && pathHexagon.satelliteData.) {
                return false
            }
        }
        return true
